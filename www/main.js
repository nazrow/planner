...
//                debugger;
        ...
        .force('links', d3.forceLink(links).id((task) => task.id).distance((link) => Math.hypot(link.source_task.x - link.target_task.x, link.source_task.y - link.target_task.y)).strength((link) => {link.distance() - 500 / 1500}));
    simulation.stop();
